...
import { ExpenseTypeFormConfigHelper } from './expense-type-form-config';


@Component({
  selector: 'daz-expense-type',
  imports: [
    CORE_IMPORTS,
    FORM_MAT_IMPORTS,
    MatDialogModule,
...
export class ExpenseTypeComponent implements OnDestroy {
  private store = inject(Store);
  private formBuilder = inject(FormBuilder);
  private dialogRef = inject(DialogRef);
  
  isProcessing = this.store.selectSignal(ExpenseTypeUIState.isProcessing);
  currentItem = this.store.selectSignal(ExpenseTypeState.getCurrent);
  
  dialogHeaderConfig = signal<DialogHeaderConfig>({ 
    title: LocaleKeys.titles.expenseType.createExpenseType 
  });
  createPermission = signal<Permission>(Permission.CAN_CREATE_EXPENSE_TYPE);
  updatePermission = signal<Permission>(Permission.CAN_UPDATE_EXPENSE_TYPE);
  ...
  constructor() {
    this.form = ExpenseTypeFormConfigHelper.createForm(this.formBuilder);
    ...
    effect(() => {
      ...
      if (isProcessing) { 
        this.dialogHeaderConfig.set({title: LocaleKeys.titles.expenseType.expenseType}); 
      }
    });
  }

  private setFormData(type: ExpenseType): void {
    if (!type) return;
    if (this.form.value.id === type.id) return;

    this.dialogHeaderConfig.set({
      title: LocaleKeys.titles.expenseType.expenseType
    });
  ...
  async onSubmit(): Promise<void> {
    ...
    const stateAction = (this.form.value.id > 0)
      ? new UpdateExpenseType(this.form.value)
      : new CreateExpenseType(this.form.value);
  ...
  ngOnDestroy(): void {
    this.store.dispatch(new ResetSelectedExpenseType());
  }
}